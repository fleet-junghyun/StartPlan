...
public class BringCalendarPickDate extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        TodayDecorator todayDecorator = new TodayDecorator(CalendarDay.from(year, month, day), BringCalendarPickDate.this);
    ...
    private void attachList(String date, CalendarDay calendarDay, String mStandardDate) {
        BringPickDateList bringPickDateList = new BringPickDateList();
        FragmentTransaction mFragmentTransaction = getSupportFragmentManager().beginTransaction();
        if (bringPickDateList.isAdded()) {
            mFragmentTransaction.remove(bringPickDateList);
            bringPickDateList = new BringPickDateList();
        }
        mFragmentTransaction.setReorderingAllowed(true);
        mFragmentTransaction.replace(R.id.fragment_pick_date_list, bringPickDateList, "pickDate_list");
        bringPickDateList.setOnFinishListener(new BringPickDateList.SetFinishListener() {
            @Override
        ...
        bundle.putString(BringPickDateList.PICK_DATE, date);
        bundle.putString(BringPickDateList.SELECTED_PICK_DATE, mStandardDate);
        try {
            bringPickDateList.setArguments(bundle);
            mFragmentTransaction.commit();
        } catch (IllegalStateException e) {
            Toast.makeText(getApplicationContext(), "오류가 발생했어요. 죄송합니다. 다시 실행해주세요.", Toast.LENGTH_SHORT).show();
        }
        if (date.equals(getToday())) {
            mSubTitle.setText(calendarDay.getMonth() + "월" + " " + calendarDay.getDay() + "일" + "(오늘)");
        } else {
